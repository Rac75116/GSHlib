...
template<class T> class RangeAddRangeSum : public decltype(internal::LazySegmentSpec([](const std::pair<T, u32>& a, const std::pair<T, u32>& b) { return std::pair<T, u32>{a.first + b.first, a.second + b.second}; }, []() -> std::pair<T, u32> { return {static_cast<T>(0), 0}; }, [](const T& f, const std::pair<T, u32>& x) { return std::pair<T, u32>{x.first + f * static_cast<T>(x.second), x.second}; }, [](const T& f, const T& g) { return f + g; }, []() -> T { return static_cast<T>(0); })){};
...
template<class T> requires std::integral<T> class RangeXorRangeXor : public decltype(internal::LazySegmentSpec([](const std::pair<T, u32>& a, const std::pair<T, u32>& b) { return std::pair<T, u32>{static_cast<T>(a.first ^ b.first), a.second + b.second}; }, []() -> std::pair<T, u32> { return {static_cast<T>(0), 0}; }, [](const T& f, const std::pair<T, u32>& x) { return std::pair<T, u32>{static_cast<T>(x.first ^ ((x.second & 1) ? f : static_cast<T>(0))), x.second}; }, [](const T& f, const T& g) { return static_cast<T>(f ^ g); }, []() -> T { return static_cast<T>(0); })){};
