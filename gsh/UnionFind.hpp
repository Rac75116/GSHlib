...
public:
    ...
    constexpr OfflinePersistentUnionfind(u32 m) : n(m) {}
    constexpr u32 size() const noexcept { return n; }
    constexpr void same(i32 k, u32 u, u32 v) { g.emplace_back(true, k + 1, u, v); }
    constexpr void merge(i32 k, u32 u, u32 v) { g.emplace_back(false, k + 1, u, v); }
    constexpr void reserve(u32 q) { g.reserve(q); }
    constexpr Arr<bool> solve() const {
        struct solver {
            ...
            Arr<u8> res;
            Arr<bool> res2;
            constexpr void dfs(u32 idx) {
                auto [t, k, u, v] = gr[idx];
                if (t) {
                    res[k - 1] = ds.same(u, v) + 1;
                } else {
                    ds.merge(u, v);
                    for (u32 i = cnt[k], j = cnt[k + 1]; i != j; ++i) dfs(i);
                    ds.undo();
                }
            }
            constexpr solver(u32 n, const Vec<query>& g) : cnt(g.size() + 2), gr(g.size() + 1), ds(n + 1), res(g.size()) {
                const u32 q = g.size();
                u32 same_cnt = 0;
                for (u32 i = 0; i != q; ++i) {
                    ++cnt[g[i].k];
                ...
                for (u32 i = 0; i != q; ++i) cnt[i + 1] += cnt[i];
                cnt[q + 1] = cnt[q];
                for (u32 i = q; i != 0; --i) {
                    u32 idx = --cnt[g[i - 1].k];
                    (gr[idx] = g[i - 1]).k = i;
