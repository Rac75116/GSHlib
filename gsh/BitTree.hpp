...
#include <type_traits>
namespace gsh {
template<u32 Size> requires (Size <= (1u << 24)) class BitTree {
protected:
  ...
  constexpr void build() noexcept {
    ...
    for(u32 x = 0; x < s3; x += 64) {
      auto get = [&](u32 n) GSH_INTERNAL_INLINE_LAMBDA -> u32 { return _mm256_movemask_pd(_mm256_castsi256_pd(_mm256_cmpeq_epi64(_mm256_load_si256(reinterpret_cast<const __m256i*>(&v3[x + n])), _mm256_setzero_si256()))); };
    ...
    for(u32 x = 0; x < s2; x += 64) {
      auto get = [&](u32 n) GSH_INTERNAL_INLINE_LAMBDA -> u32 { return _mm256_movemask_pd(_mm256_castsi256_pd(_mm256_cmpeq_epi64(_mm256_load_si256(reinterpret_cast<const __m256i*>(&v2[x + n])), _mm256_setzero_si256()))); };
      const u64 a = get(0), b = get(4), c = get(8), d = get(12), e = get(16), f = get(20), g = get(24), h = get(28);
      const u64 i = get(32), j = get(36), k = get(40), l = get(44), m = get(48), n = get(52), o = get(56), p = get(60);
...
public:
  ...
  constexpr void assign(const c8* p, u32 sz, const c8 one = '1') {
    ...
    for(u32 i = 0; i + 64 <= sz; i += 64) {
      auto get = [&](u32 n) -> u32 { return _mm256_movemask_epi8(_mm256_cmpeq_epi8(_mm256_loadu_si256(reinterpret_cast<const __m256i_u*>(p + i + n)), ones)); };
...
template<u32 Size> requires (Size <= (1u << 24)) class CountableBitTree : private BitTree<Size>, private RangeSumQuery<u32> {
