...
namespace internal {
    template<class Key, class Value, class Hasher, class Pred, class Alloc, bool Multi> class HashTable : public ViewInterface<HashTable<Key, Value, Hasher, Pred, Alloc, Multi>, std::conditional_t<std::is_void_v<Value>, Key, KeyValuePair<Key, Value>>> {
    ...
    private:
        ...
        template<class K> constexpr itype::u32 find_loc(const K& k, itype::u64 h, itype::u32 idx) const {
            itype::u32 mask = 0;
            {
                const itype::u8 tmp = h % bsize;
                const itype::u8* ptr = buckets[idx].signature;
                for (itype::u32 i = 0; i != bsize; ++i) mask |= static_cast<itype::u32>(tmp == ptr[i]) << i;
            ...
            }
            {
                ...
                while (mask != 0) {
                    ...
                    res = is_equal(data[ptr[tmp]], k) ? tmp : res;
                    mask &= mask - 1;
                }
                return res;
            }
        ...
        constexpr bool rehash_impl(itype::u32 n) {
            ...
            while (i + 8 < j) {
                ...
                for (itype::u32 k = 0; k != 8; ++k) {
                    ...
                    buckets[idx].signature[loc] = h % bsize;
        ...
        template<class K> constexpr itype::u32 insert_impl(const K& k, const itype::u64 h) {
            const itype::u32 idx = calc_index(h);
            const itype::u32 loc = find_loc(k, h, idx);
            if (loc == 0xffffffff) {
                if (buckets[idx].exist == static_cast<itype::u32>((1ull << bsize) - 1)) [[unlikely]] {
                    while (!rehash_impl(buckets.size() * 2)) {}
                    return insert_impl(h);
                }
                loc = std::countr_zero(!buckets[idx].exist);
                buckets[idx].exist |= 1u << loc;
                buckets[idx].signature[loc] = h % bsize;
                return idx * bsize + loc;
            } else if constexpr (Multi) return idx * bsize + loc;
    ...
    public:
        ...
        template<class... Args> constexpr auto emplace(Args&&... args) {
            ...
            const itype::u32 tmp = insert_impl(key, calc_hash(key));
            const itype::u32 idx = tmp / bsize, loc = tmp % bsize, n = data.size() - 1;
            if (tmp == 0xffffffff) {
                data.pop_back();
                return { begin() + n, false };
            }
            if constexpr (Multi) {
                return begin() + n;
            } else {
        ...
        constexpr auto insert(const value_type& v) {
            data.push_back(v);
            const bool f = insert_impl(data.size() - 1, calc_hash(get_key(data.back())));
            if constexpr (Multi) return begin() + data.size() - 1;
            else return { begin() + data.size() - 1, f };
        }
        constexpr auto insert(value_type&& rv) {
...
template<class Key, class Hasher = Hash, class Pred = EqualTo, class Alloc = Allocator<Key>> class HashSet : public internal::HashTable<Key, void, Hasher, Pred, Alloc, false> {};
template<class Key, class Hasher = Hash, class Pred = EqualTo, class Alloc = Allocator<Key>> class HashMultiset : public internal::HashTable<Key, void, Hasher, Pred, Alloc, true> {};
template<class Key, class Value, class Hasher = Hash, class Pred = EqualTo, class Alloc = Allocator<KeyValuePair<Key, Value>>> class HashMap : public internal::HashTable<Key, Value, Hasher, Pred, Alloc, false> {
public:
    using mapped_type = Value;
};
template<class Key, class Value, class Hasher = Hash, class Pred = EqualTo, class Alloc = Allocator<KeyValuePair<Key, Value>>> class HashMultimap : public internal::HashTable<Key, Value, Hasher, Pred, Alloc, true> {
public:
    using mapped_type = Value;
};
