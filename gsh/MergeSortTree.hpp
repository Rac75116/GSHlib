...
public:
  ...
  constexpr void on_build_begin(i64 depth, i64 n) {
    level_base.assign(depth + 1, 0);
    i64 total = 0;
    for(i64 level = 0; level != depth; ++level) {
  ...
  constexpr void build_node(i64 level, i64 node_index, range_type sorted) { aux_tree[level_base[level] + node_index] = static_cast<aux_type>(std::invoke(build_aux_func, sorted)); }
  constexpr const aux_type& aux(i64 level, i64 node_index) const { return aux_tree[level_base[level] + node_index]; }
...
public:
  ...
  constexpr void on_build_begin(i64 depth, i64 n) {
    ...
    prefix_offset.assign(total_nodes, 0);
    node_len.assign(total_nodes, 0);
    prefix.clear();
  }
  template<class Range> constexpr void build_node(i64 level, i64 node_index, Range sorted) {
    const i64 idx = level_base[level] + node_index;
    const i64 len = static_cast<i64>(sorted.size());
    node_len[idx] = len;
    prefix_offset[idx] = prefix.size();
    prefix.push_back(sum_type{});
    for(const auto& x : sorted) { prefix.push_back(static_cast<sum_type>(std::invoke(op_func, prefix.back(), x))); }
  }
  constexpr aux_type aux(i64 level, i64 node_index) const {
    const i64 idx = level_base[level] + node_index;
    const i64 off = prefix_offset[idx];
    const i64 len = node_len[idx];
    return aux_type(prefix.data() + off, prefix.data() + off + len + 1);
  }
};
} // namespace merge_sort_tree_specs
...
private:
  ...
  constexpr static bool spec_has_on_build_begin = requires { std::declval<Spec&>().on_build_begin(0, 0); };
  constexpr static bool spec_has_build_node = requires { std::declval<Spec&>().build_node(0, 0, std::declval<range_type>()); };
  constexpr static bool spec_has_on_build_end = requires { std::declval<Spec&>().on_build_end(); };
  constexpr static bool spec_has_aux = requires { std::declval<const Spec&>().aux(0, 0); };
  i64 n;
  Vec<value_type> seq;
  ...
  template<class Iter> constexpr void build(Iter iter, Iter sent) {
    n = static_cast<i64>(std::ranges::distance(iter, sent));
    if(n == 0) {
      clear();
      return;
    }
    const i64 dep = static_cast<i64>(std::bit_width(static_cast<u64>(n)));
    seq.resize(n * dep);
    ...
    for(i64 i = 0; i != dep - 1; ++i) {
      const i64 len1 = 1ll << i;
      const i64 seglen = 2ll << i;
      for(i64 j = 0; j + seglen <= n; j += seglen) {
        i64 idx1 = n * i + j;
        i64 idx2 = idx1 + len1;
        i64 idx3 = idx1 + 2 * len1;
        i64 idx4 = idx1 + n;
        i64 a = idx1, b = idx2, c = idx4;
        while(a != idx2 && b != idx3) {
...
public:
  ...
  constexpr void clear() {
    n = 0;
    seq.clear();
    if constexpr(spec_has_on_build_begin) spec.on_build_begin(0, 0);
    if constexpr(spec_has_on_build_end) spec.on_build_end();
  }
  constexpr bool empty() const { return n == 0; }
  constexpr i64 size() const { return n; }
  template<class F> constexpr void visit(i64 l, i64 r, F f) const {
#ifndef NDEBUG
    ...
    while(l < r) {
      const i64 rem = r - l;
      i64 len = static_cast<i64>(std::bit_floor(static_cast<u64>(rem)));
      if(l != 0) {
        const i64 lowbit = l & -l;
        if(lowbit < len) len = lowbit;
      }
      const i64 level = static_cast<i64>(std::countr_zero(static_cast<u64>(len)));
      const i64 base = n * level + l;
      const range_type sorted(seq.data() + base, seq.data() + base + len);
      if constexpr(spec_has_aux) {
        if constexpr(std::invocable<F, range_type, decltype(spec.aux(0, 0))>) std::invoke(f, sorted, spec.aux(level, l >> level));
        else std::invoke(f, sorted);
      } else std::invoke(f, sorted);
      l += len;
    }
  }
  constexpr i64 count_less_or_equal(i64 l, i64 r, const value_type& value) const {
    i64 ans = 0;
    visit(l, r, [&](const range_type sorted) {
      ...
      ans += static_cast<i64>(std::ranges::distance(sorted.begin(), it));
    });
    return ans;
  }
  constexpr i64 count_less_than(i64 l, i64 r, const value_type& value) const {
    i64 ans = 0;
    visit(l, r, [&](const range_type sorted) {
  ...
  constexpr i64 count_greater_or_equal(i64 l, i64 r, const value_type& value) const {
    i64 ans = 0;
    visit(l, r, [&](const range_type sorted) {
      const auto it = sorted.lower_bound(value, [&](const value_type& a, const value_type& b) { return comp(a, b); });
      const i64 less = static_cast<i64>(std::ranges::distance(sorted.begin(), it));
      ans += static_cast<i64>(sorted.size()) - less;
    });
    return ans;
  }
  constexpr i64 count_greater_than(i64 l, i64 r, const value_type& value) const {
    i64 ans = 0;
    visit(l, r, [&](const range_type sorted) {
      const auto it = sorted.upper_bound(value, [&](const value_type& a, const value_type& b) { return comp(a, b); });
      const i64 leq = static_cast<i64>(std::ranges::distance(sorted.begin(), it));
      ans += static_cast<i64>(sorted.size()) - leq;
    });
    return ans;
  }
  constexpr i64 count_equal(i64 l, i64 r, const value_type& value) const {
    i64 ans = 0;
    visit(l, r, [&](const range_type sorted) {
      ...
      ans += static_cast<i64>(std::ranges::distance(it1, it2));
    });
    return ans;
  }
};
