...
#include <utility>
namespace gsh {
template<class T, class Alloc = std::allocator<T>> requires std::same_as<T, typename std::allocator_traits<Alloc>::value_type> && std::same_as<T, std::remove_cv_t<T>> class Arr : public ViewInterface<Arr<T, Alloc>, T> {
