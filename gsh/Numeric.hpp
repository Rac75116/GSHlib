...
template<class T, class U> constexpr auto FastGCD(T x, U y) {
    static_assert(!std::is_same_v<T, bool> && !std::is_same_v<U, bool> && std::is_integral_v<T> && std::is_integral_v<U>, "gsh::fast_gcd / The input must be an integral type.");
    if constexpr (std::is_same_v<T, U>) {
        if constexpr (std::is_unsigned_v<T>) {
            if (x == 0 || y == 0) [[unlikely]]
                return x | y;
            const gsh::itype::i32 n = std::countr_zero(x);
            const gsh::itype::i32 m = std::countr_zero(y);
            const gsh::itype::i32 l = n < m ? n : m;
            x >>= n;
            y >>= m;
            T s;
            int t;
            ...
            return static_cast<T>(gsh::FastGCD<std::make_unsigned_t<T>, std::make_unsigned<T>>((x < 0 ? -x : x), (y < 0 ? -y : y)));
        }
    } else {
        return gsh::FastGCD<std::common_type_t<T, U>, std::common_type_t<T, U>>(x, y);
    }
}
// @brief Find the greatest common divisor of multiple numbers.
template<class T, class... Args> auto FastGCD(T x, Args... y) {
    return gsh::FastGCD(x, gsh::FastGCD(y...));
}
// @brief Find the  least common multiple as in std::lcm.
template<class T, class U> auto FastLCM(T x, U y) {
    return static_cast<std::common_type_t<T, U>>(x < 0 ? -x : x) / gsh::FastGCD(x, y) * static_cast<std::common_type_t<T, U>>(y < 0 ? -y : y);
}
// @brief Find the least common multiple of multiple numbers.
template<class T, class... Args> auto FastLCM(T x, Args... y) {
    return gsh::FastLCM(x, gsh::FastLCM(y...));
}

// @brief Prime number determination for 32bit numbers (https://www.techneon.com/download/is.prime.32.base.data)
bool isPrime32(const gsh::itype::u32 x) {
    ...
    using mint = gsh::DynamicModint32<-1>;
    mint::set_mod(x);
    const gsh::itype::u32 h = x * 0xad625b89;
    gsh::itype::u32 d = x - 1;
    mint cur = bases[h >> 24];
    gsh::itype::i32 s = std::countr_zero(d);
    d >>= s;
    cur = cur.pow(d);
...
// @brief Prime number determination for 64bit numbers (http://miller-rabin.appspot.com/)
bool isPrime64(const gsh::itype::u64 x) {
    if (x < 4294967296) return gsh::isPrime32(x);
    if (x % 2 == 0 || x % 3 == 0 || x % 5 == 0 || x % 7 == 0 || x % 11 == 0 || x % 13 == 0 || x % 17 == 0 || x % 19 == 0 || x % 23 == 0 || x % 29 == 0 || x % 31 == 0 || x % 37 == 0 || x % 41 == 0 || x % 43 == 0) return false;
    using mint = gsh::DynamicModint64<-1>;
    mint::set_mod(x);
    gsh::itype::u64 d = x - 1;
    const gsh::itype::i32 s = std::countr_zero(d);
    d >>= s;
    auto test = [&](gsh::itype::u64 a) -> bool {
